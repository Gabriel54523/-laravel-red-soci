...
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Editar Contacto</h1>
        <p class="text-gray-600">Modifica la información del contacto.</p>
    </div>
        ...
        <form action="{{ route('contacts.update', $contact) }}" method="POST" class="p-6 space-y-6">
            ...
            <div>
                ...
                <textarea name="status" id="status" rows="3"
                          ...
                          placeholder="Un mensaje de estado personalizado...">{{ old('status', $contact->status) }}</textarea>
                @error('status')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
