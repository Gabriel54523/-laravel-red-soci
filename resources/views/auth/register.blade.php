    ...
    <div class="max-w-md w-full space-y-8">
        ...
        <form class="mt-8 space-y-6" method="POST" action="{{ route('register') }}">
            ...
            <div class="rounded-md shadow-sm -space-y-px">
                ...
                <div>
                    <label for="phone_number" class="sr-only">Número de teléfono</label>
                    <input id="phone_number" name="phone_number" type="text" required 
                           class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm @error('phone_number') border-red-500 @enderror"
                           placeholder="Número de teléfono" value="{{ old('phone_number') }}">
                    @error('phone_number')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
